...
def main():
    ...
    target_repos = [
        repo["name"] for repo in manifest.get("repos", []) if isinstance(repo, dict)
    ]
    ...
    for repo_name in target_repos:
        ...
        ci_icon = (
            "✅"
            if details["last_ci"] == "success"
            else "❌"
            if details["last_ci"] == "failure"
            else "⚪"
        )
        ...
        report.append(
            f"| [{repo_name}](https://github.com/{full_name}) | {ci_icon} | {pr_count} | {priority_action} | {your_stale} | {stale_count} |"
        )
