...
2.  **JIT Workspace Creation**: Finding no existing project for "Roborock," the Orchestrator provisions a new **Project Workspace**: `/workspaces/roborock-integration`.
3.  **Sandbox Spawning**: A transient execution container (the "Hand") is started. It is mounted with:
    *   **Global Memory**: The user's general preferences.
    *   **Project Workspace**: The newly created (empty) folder.
4.  **Initial Tooling & Memory Encoding**:
    *   The agent uses `github_search` to locate the repository.
    *   It initializes the **`CLAUDE.md`** file (Project Memory) to store its mission statement.
...
## Current Context
- Repository: `home-assistant/roborock`
...
2.  **Hydration**: The Orchestrator spawns a new sandbox and mounts the `/workspaces/roborock-integration` folder.
3.  **Autonomous Thought**: The agent reads the `CLAUDE.md` file. It "remembers" its mission and the state of the repo from yesterday.
...
*   **Stateless Execution**: The sandbox is destroyed after every run. Memory lives in the **Filesystem (CLAUDE.md)** and the **Host Database (Binding Table)**.
